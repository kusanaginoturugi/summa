...
- 請求書の作成・一覧・PDF発行と売上伝票連携
- 見積書の作成・一覧・PDF発行（会計データとは分離）
- ダーク/ライト切替、簡易ナビゲーション
...
- 請求書: ナビの「請求書」から作成。保存時に売掛金・売上高・仮受消費税の伝票を作成します。
- 見積書: ナビの「見積書」から作成。会計データとは連携しません。

## PDF生成

請求書と見積書のPDF生成には、リポジトリ内の `invoice/invoice` を使います。
Goソースは `invoice/` にあり、再ビルドは次のコマンドで行います。
